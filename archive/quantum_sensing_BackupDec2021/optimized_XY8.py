...
def runExperiment():
	try:
		...
		#instructionArray = []
		#for i in sweep:
		#	sequenceArgs = [sweep[-1], config['t_AOM'], config['t_readoutDelay'], config['t_pi'], config['IQpadding'], int(config['N'])]
		...
		#pb_stop_programming()
		instructionArray = PBctl.programPB(config['sequence_name'], [sweep[0], config['t_AOM'], config['t_readoutDelay'], config['t_pi'], config['IQpadding'], int(config['N'])])
		...
		def animate(k):
			...
			#Otherwise, increment the frequency and continue
				...
				instructionArray = PBctl.programPB(config['sequence_name'], [sweep[step], config['t_AOM'], config['t_readoutDelay'], config['t_pi'], config['IQpadding'], int(config['N'])])
