...
def runExperiment():
    try:
        ...
        sweep, step_size = np.linspace(config['START_LENGTH'], config['END_LENGTH'], num = config['N_pts'], retstep = True)
        ...
        # pb_start()
        makeSingleRabiSeq(inst, config['t_AOM'], config['MW_FREQ'], config['t_readoutDelay'], config['START_LENGTH'])
        ...
        def animate(k):
            ...
            #Otherwise, increment the frequency and continue
                ...
                makeSingleRabiSeq(inst, config['t_AOM'], config['MW_FREQ'], config['t_readoutDelay'], t_duration)
                ...
                plt.title('Photo Counter Readout vs MW Duration')
                plt.ylabel('Photon Counts')
                plt.xlabel('MW Duration (us)')
