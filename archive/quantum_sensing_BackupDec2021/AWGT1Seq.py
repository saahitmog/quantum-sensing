...
def squarePulseOffset(segmentLength, bits, squareCycles, duty, amp, offset):
    time = np.linspace(0, segmentLength-1, segmentLength)
    omegaSquare = 2 * np.pi * squareCycles
    squareSignal = np.roll((sg.square(omegaSquare*time/segmentLength, duty)+1)/2, offset)
    squareSignal[:offset] = [0] * offset
...
def makeSingleT1Seq(inst, duration, t_readoutDelay, t_pi):
    segmentLength = 4999936
    segmentLength = 8998848 #this segment length is optimized for 1kHz trigger signal
    squares = int((1/(duration / 1e6)) * segmentLength / 9e9)
    duty = 0.5
    t_readoutDelay = int((segmentLength / duration) * t_readoutDelay)
    print('Duration: {0} us'.format(duration))
...
def runExperiment():
    try:
        ...
        # pb_start()
        makeSingleT1Seq(inst, config['t_readoutDelay'], config['start_t'], config['t_pi'])
        ...
        def animate(k):
            ...
            #Otherwise, increment the frequency and continue
                ...
                makeSingleT1Seq(inst, config['t_readoutDelay'], f, config['t_pi'])
                ...
                plt.title('Photo Counter Readout vs Duration')
