...
def makeSingleSpinEchoSeq(inst, duration, freq, t_delay, t_pi, tau):
    ...
    instrumentCalls2(inst, squareWave(segmentLength, 8, 10, 1))
    instrumentCalls2(inst, sinePulse(segmentLength, 8, 5, cycles, 0.5, 1))
...
def runExperiment():
    try:
        ...
        makeSingleSpinEchoSeq(inst, config['t_AOM'], config['MW_FREQ'], config['t_readoutDelay'], config['t_pi'], config['start_tau'])
        ...
        def animate(k):
            ...
            #Otherwise, increment the frequency and continue
                ...
                makeSingleSpinEchoSeq(inst, config['t_AOM'], config['MW_FREQ'], config['t_readoutDelay'], config['t_pi'], f)
