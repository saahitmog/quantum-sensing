...
class ESRMeasure(Measurement):
    ...
    def setup(self):
        ...
        self.settings.New('Start_Frequency', dtype=float, initial=3.088, spinbox_decimals=4, unit='GHz')        
        self.settings.New('End_Frequency', dtype=float, initial=3.1, spinbox_decimals=4, unit='GHz')      
        self.settings.New('Vpp', dtype=float, initial=0.2, spinbox_decimals=3, spinbox_step=0.001, unit='V')
        ...
        self.settings.New('t_AOM', dtype=int, initial=500, unit='us')
        self.settings.New('magnet_power', dtype=float, initial=0.1)
            ...
            for j in range(self.settings.Navg.value):
                ...
                for step in range(0, len(sweep)):
                    ...
                    if(mode == 'AWG'):
                        ...
                        makeSingleESRSeq(inst, self.settings.t_AOM.value, f, self.settings.Vpp.value)
                    else:    
...
class RabiMeasure(Measurement):
    ...
    def setup(self):
        ...
        for rate in self.states:
            self.settings.New('{}_visible'.format(rate), dtype=bool, initial=True)   
        self.settings.New('photon_mode', dtype=bool, initial=False)
        self.settings.New('AWG_mode', dtype=bool, initial=False)
        self.settings.New('Frequency', dtype=float, initial=2.7)  
        self.settings.New('Start_Duration', dtype=float, initial=20)        
        self.settings.New('End_Duration', dtype=float, initial=240)      
        self.settings.New('Vpp', dtype=float, initial=0.001, spinbox_decimals=4, spinbox_step=0.001)
        self.settings.New('N_pts', dtype=int, initial=101)
        self.settings.New('N_samples', dtype=int, initial=1000)
        self.settings.New('DAQtimeout', dtype=int, initial=10)
        self.settings.New('t_AOM', dtype=int, initial=200)
        self.settings.New('mw_power', dtype=float, initial=0.16)
        self.settings.New('magnet_power', dtype=float, initial=0.1)
        self.settings.New('Navg', dtype=int, initial=3) 
        self.settings.New('figsize_x', dtype=int, initial=9)
        self.settings.New('figsize_y', dtype=int, initial=5)
        self.settings.New('plotting_type', dtype=str, initial='signal')
        self.settings.New('NThrow', dtype=int, initial=0)
        self.settings.New('fname_format', dtype=str, initial='{timestamp:%y%m%d_%H%M%S}_{measurement.name}_{sample}.{ext}')
    ...
    def setup_figure(self):        
        ...
        self.graph_layout = pg.GraphicsLayoutWidget()
        self.ui.channel_optimizer_GroupBox.layout().addWidget(self.graph_layout)
        self.plot = self.graph_layout.addPlot(title="Signal vs MW Pulse Length")
    ...
    def run(self):
        ...
        #sweep is a np array of frequency values that we will sweep across, step_size is the size of steps between
        sweep, step_size = np.linspace(self.settings.Start_Duration.value, self.settings.Duration.value, num = self.settings.N_pts.value, retstep = True)
        #sweep = np.arange(config['START_FREQUENCY'], config['END_FREQUENCY'] + config['STEP_FREQUENCY'], config['STEP_FREQUENCY'])
        print(sweep)
        #create a dictionary that will save signal and background information and will be exported to pandas df and then to a csv
        save_dict = {}
        ...
        try:
            ...
            # #initiate RF generator with channel and model type; set amplitude
                ...
                SRSctl.setSRS_RFAmplitude(my_SRS, self.settings.Vpp.value, units = "Vpp")
                # #set test frequency and modulation; ESR has no modulation but that's handled in the modulation function
                SRSctl.setSRS_Freq(my_SRS, self.settings.Frequency.value, 'GHz') 
                SRSctl.setupSRSmodulation(my_SRS, 'RabiSeq') 
                SRSctl.enableSRS_RFOutput(my_SRS)
                print('SRS Initialized')
            #run the initPB function from the PBinit file 
            # # initPB()
...
def runExperiment():
    try:
        ...
        # pb_start()
        makeSingleESRSeq(inst, self.settings.t_AOM.value, self.settings.Start_Frequency.value, self.settings.Vpp.value)
