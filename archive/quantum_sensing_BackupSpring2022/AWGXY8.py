...
def instrumentCalls2(inst, waveform, offset=0):
    query_syst_err = True  
    ...
    # select channel
    SendScpi(inst, ":INST:CHAN 2", query_syst_err)
...
def sinePulseEcho(segmentLength, sinCycles, amp, t_pi, tau, t_delay = 0):
    time = np.linspace(0, segmentLength-1, segmentLength)
    omegaSin = 2 * np.pi * sinCycles
    #squaresig = np.zeros(t_delay) + np.ones(t_pi/2) + np.zeros(tau) + np.ones(t_pi) + np.zeros(tau) + np.ones(t_pi/2)
    #squareSignal = (sg.square(50*time/segmentLength, 0.5)+1)/2
    squareSignal = (sg.square(50*time/segmentLength, 1) * 0)
    t_pihalf = int(t_pi/2)
    squareSignal[:t_delay] = [0] * t_delay
    squareSignal[t_delay:(t_delay+t_pihalf)] = [1] * (t_pihalf)
    squareSignal[(t_delay+t_pihalf):(t_delay+t_pihalf+tau)] = [0] * tau
    squareSignal[(t_delay+t_pihalf+tau):(t_delay+t_pihalf+t_pi+tau)] = [1] * t_pi
    squareSignal[(t_delay+t_pihalf+t_pi+tau):(t_delay+t_pihalf+t_pi+2*tau)] = [0] * tau
    squareSignal[(t_delay+t_pihalf+t_pi+2*tau):(t_delay+2*t_pihalf+t_pi+2*tau)] = [1] * (t_pihalf)
    rawSignal = (squareSignal*amp*np.sin(omegaSin*time/segmentLength))
    dacSignal = scaleWaveform(rawSignal, "P9082M")

    return(dacSignal);

def sinePulseXY8(segmentLength, sinCycles, amp, t_pi, tau, t_delay = 0):
    time = np.linspace(0, segmentLength-1, segmentLength)
    omegaSin = 2 * np.pi * sinCycles
    #squaresig = np.zeros(t_delay) + np.ones(t_pi/2) + np.zeros(tau) + np.ones(t_pi) + np.zeros(tau) + np.ones(t_pi/2)
    #squareSignal = (sg.square(50*time/segmentLength, 0.5)+1)/2
    ...
    squareSignal[:t_delay] = [0] * t_delay
    squareSignal[t_delay:(t_delay+t_pihalf)] = [1] * (t_pihalf)
    squareSignal[(t_delay+t_pihalf):(t_delay+t_pihalf+tauhalf)] = [0] * tauhalf
    squareSignal[(t_delay+t_pihalf+tauhalf):(t_delay+t_pihalf+t_pi+tauhalf)] = [1] * t_pi
    squareSignal[(t_delay+t_pihalf+t_pi+tauhalf):(t_delay+t_pihalf+t_pi+tauhalf+tau)] = [0] * tau
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi+tau):(t_delay+t_pihalf+tauhalf+t_pi*2+tau)] = [1] * t_pi
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*2+tau):(t_delay+t_pihalf+tauhalf+t_pi*2+tau*2)] = [0] * tau
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*2+tau*2):(t_delay+t_pihalf+tauhalf+t_pi*3+tau*2)] = [1] * t_pi
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*3+tau*2):(t_delay+t_pihalf+tauhalf+t_pi*3+tau*3)] = [0] * tau
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*3+tau*3):(t_delay+t_pihalf+tauhalf+t_pi*4+tau*3)] = [1] * t_pi
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*4+tau*3):(t_delay+t_pihalf+tauhalf+t_pi*4+tau*4)] = [0] * tau
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*4+tau*4):(t_delay+t_pihalf+tauhalf+t_pi*5+tau*4)] = [1] * t_pi
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*5+tau*4):(t_delay+t_pihalf+tauhalf+t_pi*5+tau*5)] = [0] * tau
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*5+tau*5):(t_delay+t_pihalf+tauhalf+t_pi*6+tau*5)] = [1] * t_pi
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*6+tau*5):(t_delay+t_pihalf+tauhalf+t_pi*6+tau*6)] = [0] * tau
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*6+tau*6):(t_delay+t_pihalf+tauhalf+t_pi*7+tau*6)] = [1] * t_pi
    squareSignal[(t_delay+t_pihalf+tauhalf+t_pi*7+tau*6):(t_delay+t_pihalf+tauhalf*2+t_pi*7+tau*6)] = [0] * tauhalf
    squareSignal[(t_delay+t_pihalf+tauhalf*2+t_pi*7+tau*6):(t_delay+t_pihalf*2+tauhalf*2+t_pi*7+tau*6)] = [1] * (t_pihalf)
    rawSignal = (squareSignal*amp*np.sin(omegaSin*time/segmentLength))
    dacSignal = scaleWaveform(rawSignal, "P9082M")

    return(dacSignal);

def squareWave(segmentLength, aom, off, amp):
    time = np.linspace(0, segmentLength-1, segmentLength)
    duty = aom/(aom+off)
    squareSignal = sg.square(time/segmentLength, duty)
    rawSignal = (squareSignal*amp)
    dacSignal = scaleWaveform(rawSignal, "P9082M")

    return(dacSignal);

def makeSingleSpinEchoSeq(inst, duration, freq, t_delay, t_pi, tau):
    segmentLength = 8998848
    print('Tau: {0} us'.format(tau))
    cycles = int(freq * segmentLength * 1e6 / 9e9)
    offset = int((segmentLength / duration) * t_delay)
    t_pi = int((segmentLength / duration) * t_pi)
    tau = int((segmentLength/duration) * tau)

    instrumentCalls(inst, sinePulseEcho(segmentLength, cycles, 1, t_pi, tau, offset))

    
def makeSingleXY8Seq(inst, duration, freq, t_delay, t_pi, tau):
    segmentLength = 8998848
    print('Tau: {0} us'.format(tau))
    cycles = int(freq * segmentLength * 1e6 / 9e9)
    offset = int((segmentLength / duration) * t_delay)
    ...
    instrumentCalls(inst, sinePulseXY8(segmentLength, cycles, 1, t_pi, tau, offset))
    instrumentCalls2(inst, squareWave(segmentLength, 50, 50, 1))
...
def runExperiment():
    try:
        ...
        sweep, step_size = np.linspace(config['start_tau'], config['end_tau'], num = config['N_pts'], retstep = True)
        ...
        makeSingleXY8Seq(inst, config['t_AOM'], config['MW_FREQ'], config['t_readoutDelay'], config['t_pi'], config['start_tau'])
        ...
        def animate(k):
            ...
            #Otherwise, increment the frequency and continue
                ...
                makeSingleXY8Seq(inst, config['t_AOM'], config['MW_FREQ'], config['t_readoutDelay'], config['t_pi'], f)
                ...
                plt.title('Photo Counter Readout vs Tau Duration')
                plt.ylabel('Photon Counts')
                plt.xlabel('Tau Duration (us)')
