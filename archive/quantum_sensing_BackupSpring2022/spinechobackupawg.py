...
def sinePulseOffset(segmentLength, bits, squareCycles, sinCycles, duty, amp, offset):
    ...
    omegaSquare = 2 * np.pi * squareCycles
    omegaSin = 2 * np.pi * sinCycles
    squareSignal = np.roll((sg.square(omegaSquare*time/segmentLength, duty)+1)/2, offset) #OPTIMIZE
...
def makeSingleReadoutDelaySeq(inst, duration, freq, t_delay):
    segmentLength = 8998848
    cycles = int(freq * segmentLength * 1e6 / 9e9)
    squares = int((1/(duration / 1e6)) * segmentLength / 9e9)
    duty = 0.5
    offset = int((segmentLength / duration) * t_delay)
    print('Delay: {0} us'.format(t_delay))
    instrumentCalls(inst, sinePulseOffset(segmentLength, 8, squares, cycles, duty, 1, offset))

def makeSingleRabiSeq(inst, duration, freq, t_delay, mw_duration):
    segmentLength = 8998848
    cycles = int(freq * segmentLength * 1e6 / 9e9)
    squares = int((1/(duration / 1e6)) * segmentLength / 9e9)
    duty = mw_duration / duration
    offset = int((segmentLength / duration) * t_delay)
    print('Duration: {0} us'.format(mw_duration))
    instrumentCalls(inst, sinePulseOffset(segmentLength, 8, squares, cycles, duty, 1, offset))

def sinePulseEcho(segmentLength, sinCycles, amp, t_pi, tau, t_delay = 0):
    """ time = np.linspace(0, segmentLength-1, segmentLength)
    omegaSin = 2 * np.pi * sinCycles
    #squareSignal = np.zeros(t_delay) + np.ones(t_pi/2) + np.zeros(tau) + np.ones(t_pi) + np.zeros(tau) + np.ones(t_pi/2)
    squareSignal = (sg.square(5*time/segmentLength, 0.5)+1)/2
    rawSignal = (squareSignal*amp*np.sin(omegaSin*time/segmentLength))
    dacSignal = scaleWaveform(rawSignal, "P9082M")

    return(dacSignal);  """
    time = np.linspace(0, segmentLength-1, segmentLength)
    omegaSquare = 2 * np.pi * 5
    omegaSin = 2 * np.pi * sinCycles
    squareSignal = np.roll((sg.square(omegaSquare*time/segmentLength, 0.5)+1)/2, t_delay) #OPTIMIZE
...
def runExperiment():
    try:
        ...
        # pb_start()
        #makeSingleESRSeq(inst, config['t_AOM'], config['MW_FREQ'])
        print('debug')
        makeSingleSpinEchoSeq(inst, config['t_AOM'], config['MW_FREQ'], config['t_readoutDelay'], config['t_pi'], config['tau']) #FIX
