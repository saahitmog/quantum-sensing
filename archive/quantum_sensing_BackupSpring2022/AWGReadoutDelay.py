...
def runExperiment():
    try:
        ...
        sweep, step_size = np.linspace(config['START_DELAY'], config['END_DELAY'], num = config['N_pts'], retstep = True)
        ...
        # pb_start()
        makeSingleReadoutDelaySeq(inst, config['t_AOM'], config['MW_FREQ'], config['START_DELAY'])
        ...
        def animate(k):
            ...
            #Otherwise, increment the frequency and continue
                ...
                makeSingleReadoutDelaySeq(inst, config['t_AOM'], config['MW_FREQ'], t_delay)
                ...
                plt.title('Photo Counter Readout vs Readout Delay')
