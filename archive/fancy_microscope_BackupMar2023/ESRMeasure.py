            ...
            for j in range(self.settings.Navg.value):
                ...
                for step in range(0, len(sweep)):
                    ...
                    if(mode == 'AWG'):
                        ...
                        #     AWGctl.makeSingleESRSeq(inst, self.settings.t_duration.value, f, self.settings.Vpp.value)
                        # else:
                        #     AWGctl.makeSingleESRSeqFast(inst, self.settings.t_duration.value, f, self.settings.Vpp.value)
                    ...
                    lasttime=currtime
                    currtime=time.time()
                    print('Generate sequence:', currtime-lasttime, ' seconds')
                    #Read from the DAQ
