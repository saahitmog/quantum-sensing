...
def instrumentCallsMarker(inst, waveform, vpp=0.001, offset=0):
    query_syst_err = True  
...
def makeMarker(inst, segmentLength):
    ...
    # enble marker 1 CH 1
    offTime = int(segmentLength/16)
    onTime = int(segmentLength/16)
    markerWave = []
...
def makeESRMarker(inst, segmentLength):
    ...
    for i in range(int(delay*segmentLength/1e6)+int((delay-duration)*segmentLength/1e6), int(delay*segmentLength/1e6)+int(delay*segmentLength/1e6)):
        waveform[i] = 1

    rawSignal = waveform
    dacSignal = scaleWaveform(rawSignal, "P9082M")

    markerWave = np.uint8(dacSignal)
...
def makeSingleESRSeqMarker(inst, duration, freq, vpp = 0.001):
    ...
    duty = 0.5
    print('Frequency: {0} GHz'.format(freq))
    instrumentCallsMarker(inst, sinePulse(segmentLength, 8, squares, cycles, duty, 1), vpp)
...
def makeSingleRabiSeq(inst, mw_duration, mw_delay, freq, vpp=0.001):
    segmentLength = 4999936
    segmentLength = 8998848 #this segment length is optimized for 1kHz trigger signal
    cycles = int(freq * segmentLength * 1e9 / 9e9)
