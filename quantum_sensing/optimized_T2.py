...
def runExperiment():
	try:
		...
		#instructionArray = []
		#for i in sweep:
		#	sequenceArgs = [i*ns, config['t_AOM'] * ns, config['t_readoutDelay'] * ns, config['t_pi'], config['IQpadding'], config['numberOfPiPulses']]
		#	instructionArray.extend(PBctl.programPB(config['sequence_name'], sequenceArgs))
		#print(instructionArray)

		instructionArray = PBctl.programPB(config['sequence_name'], [sweep[0], config['t_AOM'], config['t_readoutDelay'], config['t_pi'], config['IQpadding'], config['numberOfPiPulses']])
		...
		def animate(k):
			...
			#Otherwise, increment the frequency and continue
				...
				instructionArray = PBctl.programPB(config['sequence_name'], [sweep[step], config['t_AOM'], config['t_readoutDelay'], config['t_pi'], config['IQpadding'], config['numberOfPiPulses']])
