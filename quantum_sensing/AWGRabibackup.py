...
def runExperiment():
    try:
        ...
        #pb_start()
        makeSingleReadoutDelaySeq(inst, config['t_AOM'], config['MW_FREQ'], config['t_delay'])
        ...
        def animate(k):
            ...
            #Otherwise, increment the frequency and continue
                ...
                mw_duration = sweep[step]

                #init RF generator with new frequency
                # SRSctl.setSRS_Freq(my_SRS, f, 'GHz')
                #init AWG with new frequency
                makeSingleRabiSeq(inst, config['t_AOM'], config['MW_FREQ'], config['t_delay'], mw_duration)
