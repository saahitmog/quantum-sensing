...
import numpy as np
import sys
#import DAQcontrol_stream_read as DAQ #NOTE THIS CHANGE FROM PHOTODIODE CODE
    ...
    try:
        ...
        def save():
            ...
            if saved == 'y':
                ...
                name = config['folder'] + '/' + config['sequence_name'] + datetime.datetime.fromtimestamp(time.time()).strftime('%y%m%d_%H%M%S') + '_' + str(config['magnet_power'])
                #save the figure
