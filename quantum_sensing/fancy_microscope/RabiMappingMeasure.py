...
class RabiImageMeasure(Measurement):
    ...
    def setup(self):
        ...
        S.New('Start_Duration', dtype=float, initial=1e-7, unit='s', si=True)
        S.New('End_Duration', dtype=float, initial=2e-7, unit='s', si=True)      
    ...
    def run(self):
        ...
        sweep, step = np.linspace(S.Start_Duration.value, S.End_Duration.value, num = S.N_pts.value, retstep = True)
        # print(sweep)
            ...
            for idx, pix in enumerate(xy):
                ...
                for n in range(S.Navg.value):
                    ...
                    for i, f in enumerate(sweep):
                        ...
                        AWGctl.makeSingleRabiSeqMarker(self.inst, f, S.MW_delay.value, S.MW_Frequency.value, S.Vpp.value)
