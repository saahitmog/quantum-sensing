...
class T2SweepMeasure(Measurement):
    ...
    def setup(self):
        ...
        self.settings.New('Start_Tau', dtype=float, initial=536e-9, spinbox_decimals=3, unit='s', si=True)
        self.settings.New('End_Tau', dtype=float, initial=2e-4, spinbox_decimals=3, unit='s', si=True)      
        ...
        self.settings.New('t_pi', dtype=float, initial=50e-9, unit='s', si=True)
        ...
        self.settings.New('Navg', dtype=int, initial=5)
    ...
    def run(self):
        ...
        sweep, step_size = np.linspace(self.settings.Start_Tau.value, self.settings.End_Tau.value, num = self.settings.N_pts.value, retstep = True)
        ...
        try:
            ...
            AWGctl.makeT2Sweep(inst, self.settings.t_AOM.value, self.settings.t_pi.value, self.settings.t_readoutDelay.value, sweep, self.settings.MW_Frequency.value, self.settings.Vpp.value)
            ...
            for j in range(self.settings.Navg.value):
                ...
                #     return
                n = len(sweep)
                counts = DAQ.readDAQ(task, self.settings.N_samples.value * 2 * n, self.settings.DAQtimeout.value) # READS 2 * N_samples * N_pts samples from DAQ
                # print(len(counts))
                lasttime=currtime
                currtime=time.time()
    ...
    def update_display(self):
        ...
        if(self.settings.plotting_type.value != 'signal'):
            self.plot.setTitle("Contrast vs Tau")
        else:
