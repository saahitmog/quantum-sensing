...
def runExperiment():
	try:
		config['IQpadding'] = config['t_min']*round(30*ns/config['t_min'])
		print(config['t_min'])
		...
		sweep, step_size = np.linspace(config['start_tcorr'], config['end_tcorr'], num = config['N_pts'], retstep = True)
		...
		#instructionArray = []
		#for i in sweep:
		#	sequenceArgs = [i * ns, config['tau0'] * ns, config['t_AOM'] * ns, config['t_readoutDelay'] * ns, config['t_pi'] * ns, config['IQpadding'] * ns, int(config['N'])]
		#	instructionArray.extend(PBctl.programPB(config['sequence_name'], sequenceArgs))
		#print(instructionArray)
		instructionArray = PBctl.programPB(config['sequence_name'], [sweep[0], config['tau0'], config['t_AOM'], config['t_readoutDelay'], config['t_pi'], config['IQpadding'], int(config['N'])])
		...
		def animate(k):
			...
			#Otherwise, increment the frequency and continue
				...
				instructionArray = PBctl.programPB(config['sequence_name'], [sweep[step], config['tau0'], config['t_AOM'], config['t_readoutDelay'], config['t_pi'], config['IQpadding'], int(config['N'])])
